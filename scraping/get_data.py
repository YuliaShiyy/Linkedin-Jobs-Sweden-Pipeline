        ...
        try:
            ...
            job_detail_elements = wait.until(
                EC.visibility_of_all_elements_located(
                    (By.XPATH, "//div[contains(@class, 'job-details-fit-level-preferences')]")
                )
            )
            job_detail = [el.text.strip() for el in job_detail_elements]
        except:
            try:
                # mode 2
                li_elements = wait.until(
                    EC.visibility_of_all_elements_located(
                        (By.XPATH, "//div[contains(@class, 'mt2 mb2')]/ul/li")
                    )
                )
                if len(li_elements) > 1:
                    job_detail = "\n".join(li.text.strip() for li in li_elements[:-1])  # 排除最后一个
                else:
                    job_detail = li_elements[0].text.strip() if li_elements else "N/A"
            except:
                job_detail = "N/A"
